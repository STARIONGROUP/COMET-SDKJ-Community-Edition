...
/**
 * Class used for the verification if a {@link OrExpression} is compliant to data in an {@link
 * Iteration}
 */
@Log4j2
public class OrExpressionVerifier extends BooleanExpressionVerifierBase<OrExpression> {

  /**
   * Initializes a new instance of the {@link OrExpressionVerifier} class.
   *
   * @param orExpression The {@link OrExpression} that is verified.
   */
  public OrExpressionVerifier(OrExpression orExpression) {
    this.setExpression(orExpression);
  }
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      this.setRequirementStateOfCompliance(requirementStateOfCompliances.stream()
          ...
          : requirementStateOfCompliances.stream()
              .anyMatch(x -> x == RequirementStateOfCompliance.Pass)
