/*
 * ExclusiveOrExpressionVerifier.java
 *
...
/**
 * Class used for the verification if a {@link ExclusiveOrExpression} is compliant to data in an
 * {@link Iteration}
 */
@Log4j2
public class ExclusiveOrExpressionVerifier extends
    BooleanExpressionVerifierBase<ExclusiveOrExpression> {

  /**
   * Initializes a new instance of the {@link ExclusiveOrExpressionVerifier} class.
   *
   * @param exclusiveOrExpression The {@link ExclusiveOrExpression} that is verified.
   */
  public ExclusiveOrExpressionVerifier(ExclusiveOrExpression exclusiveOrExpression) {
    this.setExpression(exclusiveOrExpression);
  }
  ...
  public CompletableFuture<RequirementStateOfCompliance> verifyRequirementStateOfCompliance(
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      this.setRequirementStateOfCompliance(requirementStateOfCompliances.stream()
          ...
          : requirementStateOfCompliances.stream()
              .filter(x -> x == RequirementStateOfCompliance.Pass).count() == 1
