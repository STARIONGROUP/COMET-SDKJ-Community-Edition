...
public abstract class AbstractNestedElement extends AbstractThing implements NamedThing, OwnedThing, ShortNamedThing, VolatileThing {
    /**
     ...
     * @see NestedElement
     */
    public AbstractNestedElement(UUID iid, ConcurrentHashMap<Pair<UUID, UUID>, Lazy<Thing>> cache, URI iDalUri) {
        this.elementUsage = new OrderedItemList<ElementUsage>(this);
        this.nestedParameter = new ContainerList<NestedParameter>(this);
    }
    ...
    protected Thing genericClone(boolean cloneContainedThings) throws CloneNotSupportedException {
        ...
        clone.setElementUsage(new OrderedItemList<ElementUsage>(this.elementUsage, this));
        clone.setExcludedDomain(new List<DomainOfExpertise>(this.excludedDomain));
        clone.setExcludedPerson(new List<Person>(this.excludedPerson));
        clone.setNestedParameter(cloneContainedThings ? new ContainerList<NestedParameter>(clone) : new ContainerList<NestedParameter>(this.nestedParameter, clone));

        if (cloneContainedThings) {
            clone.getNestedParameter().addAll(this.nestedParameter.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
        }
    ...
    void resolveProperties(cdp4common.dto.Thing dtoThing) {
        ...
        this.elementUsage.resolveList(dto.getElementUsage(), dto.getIterationContainerId(), this.cache);
        this.excludedDomain.resolveList(dto.getExcludedDomain(), dto.getIterationContainerId(), this.cache);
        this.excludedPerson.resolveList(dto.getExcludedPerson(), dto.getIterationContainerId(), this.cache);
        this.isVolatile = dto.getIsVolatile();
        this.modifiedOn = dto.getModifiedOn();
        this.nestedParameter.resolveList(dto.getNestedParameter(), dto.getIterationContainerId(), this.cache);
        this.revisionNumber = dto.getRevisionNumber();
        this.rootElement = this.cache.get<ElementDefinition>(dto.getRootElement(), dto.getIterationContainerId()) ?? SentinelThingProvider.getSentinel<ElementDefinition>();
    ...
    public cdp4common.dto.Thing toDto() {
        cdp4common.dto.NestedElement dto = new cdp4common.dto.NestedElement(this.iid, this.revisionNumber);

        dto.getElementUsage().add(this.elementUsage.toDtoOrderedItemList());
        dto.getExcludedDomain().add(this.excludedDomain.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.getExcludedPerson().add(this.excludedPerson.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.setIsVolatile(this.isVolatile);
        dto.setModifiedOn(this.modifiedOn);
        dto.getNestedParameter().add(this.nestedParameter.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.setRevisionNumber(this.revisionNumber);
        dto.setRootElement(this.rootElement != null ? this.rootElement.getIid() : new UUID(0L, 0L));
