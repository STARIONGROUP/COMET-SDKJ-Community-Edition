 ...
 */
public abstract class AbstractEngineeringModel extends AbstractTopContainer  {
    /**
     ...
     */
    public AbstractEngineeringModel() {
     ...
     * @see EngineeringModel
     */
    public AbstractEngineeringModel(UUID iid, ConcurrentHashMap<Pair<UUID, UUID>, Lazy<Thing>> cache, URI iDalUri) {
        this.book = new OrderedItemList<Book>(this, true);
        this.commonFileStore = new ContainerList<CommonFileStore>(this);
        this.genericNote = new ContainerList<EngineeringModelDataNote>(this);
        this.iteration = new ContainerList<Iteration>(this);
    ...
    protected Thing genericClone(boolean cloneContainedThings) throws CloneNotSupportedException {
        ...
        clone.setBook(cloneContainedThings ? new OrderedItemList<Book>(clone, true) : new OrderedItemList<Book>(this.book, clone));
        clone.setCommonFileStore(cloneContainedThings ? new ContainerList<CommonFileStore>(clone) : new ContainerList<CommonFileStore>(this.commonFileStore, clone));
        clone.setExcludedDomain(new List<DomainOfExpertise>(this.excludedDomain));
        clone.setExcludedPerson(new List<Person>(this.excludedPerson));
        clone.setGenericNote(cloneContainedThings ? new ContainerList<EngineeringModelDataNote>(clone) : new ContainerList<EngineeringModelDataNote>(this.genericNote, clone));
        clone.setIteration(cloneContainedThings ? new ContainerList<Iteration>(clone) : new ContainerList<Iteration>(this.iteration, clone));
        clone.setLogEntry(cloneContainedThings ? new ContainerList<ModelLogEntry>(clone) : new ContainerList<ModelLogEntry>(this.logEntry, clone));
        clone.setModellingAnnotation(cloneContainedThings ? new ContainerList<ModellingAnnotationItem>(clone) : new ContainerList<ModellingAnnotationItem>(this.modellingAnnotation, clone));

        if (cloneContainedThings) {
            clone.getBook().addAll(this.book.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
            clone.getCommonFileStore().addAll(this.commonFileStore.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
            clone.getGenericNote().addAll(this.genericNote.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
            clone.getIteration().addAll(this.iteration.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
            clone.getLogEntry().addAll(this.logEntry.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
            clone.getModellingAnnotation().addAll(this.modellingAnnotation.stream().map(x -> x.Clone(true)).collect(Collectors.toList());
        }
    ...
    protected Iterable<String> validatePocoCardinality() {
        ...
        if (this.engineeringModelSetup == null || this.engineeringModelSetup.getIid().equals(new UUID(0L, 0L))) {
        ...
        int iterationCount = this.iteration.size();
        if (iterationCount < 1) {
    ...
    void resolveProperties(cdp4common.dto.Thing dtoThing) {
        ...
        this.book.resolveList(dto.getBook(), dto.getIterationContainerId(), this.cache);
        this.commonFileStore.resolveList(dto.getCommonFileStore(), dto.getIterationContainerId(), this.cache);
        this.engineeringModelSetup = this.cache.get<EngineeringModelSetup>(dto.getEngineeringModelSetup(), dto.getIterationContainerId()) ?? SentinelThingProvider.getSentinel<EngineeringModelSetup>();
        this.excludedDomain.resolveList(dto.getExcludedDomain(), dto.getIterationContainerId(), this.cache);
        this.excludedPerson.resolveList(dto.getExcludedPerson(), dto.getIterationContainerId(), this.cache);
        this.genericNote.resolveList(dto.getGenericNote(), dto.getIterationContainerId(), this.cache);
        this.iteration.resolveList(dto.getIteration(), dto.getIterationContainerId(), this.cache);
        ...
        this.modellingAnnotation.resolveList(dto.getModellingAnnotation(), dto.getIterationContainerId(), this.cache);
    ...
    public cdp4common.dto.Thing toDto() {
        cdp4common.dto.EngineeringModel dto = new cdp4common.dto.EngineeringModel(this.iid, this.revisionNumber);

        dto.getBook().add(this.book.toDtoOrderedItemList());
        dto.getCommonFileStore().add(this.commonFileStore.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.setEngineeringModelSetup(this.engineeringModelSetup != null ? this.engineeringModelSetup.getIid() : new UUID(0L, 0L));
        dto.getExcludedDomain().add(this.excludedDomain.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.getExcludedPerson().add(this.excludedPerson.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.getGenericNote().add(this.genericNote.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.getIteration().add(this.iteration.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.setLastModifiedOn(this.lastModifiedOn);
        dto.getLogEntry().add(this.logEntry.stream().map(x -> x.Iid).collect(Collectors.toList()));
        dto.getModellingAnnotation().add(this.modellingAnnotation.stream().map(x -> x.Iid).collect(Collectors.toList()));
